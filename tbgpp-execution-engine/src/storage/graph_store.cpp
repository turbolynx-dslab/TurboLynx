...
StoreAPIResult iTbgppGraphStore::InitializeScan(std::queue<ExtentIterator *> &ext_its, LabelSet labels, std::vector<LabelSet> edgeLabels, LoadAdjListOption loadAdj, PropertyKeys properties, std::vector<duckdb::LogicalType> scanSchema) {
...
StoreAPIResult iTbgppGraphStore::doScan(std::queue<ExtentIterator *> &ext_its, DataChunk& output, LabelSet labels, std::vector<LabelSet> edgeLabels, LoadAdjListOption loadAdj, PropertyKeys properties, std::vector<duckdb::LogicalType> scanSchema) {
...
StoreAPIResult iTbgppGraphStore::doScan(std::queue<ExtentIterator *> &ext_its, duckdb::DataChunk &output, vector<vector<uint64_t>> &projection_mapping, std::vector<duckdb::LogicalType> &scanSchema, std::string filterKey, duckdb::Value filterValue) {
	ExtentID current_eid;
	auto ext_it = ext_its.front();
	vector<idx_t> output_column_idxs; // TODO this should be generated in physical planning step
	bool scan_ongoing = ext_it->GetNextExtent(client, output, current_eid, filterKey, filterValue, output_column_idxs, scanSchema);
	if (scan_ongoing) {
		//output.Reference(*output_);
		return StoreAPIResult::OK;
	} else {
...
StoreAPIResult iTbgppGraphStore::doVertexIndexSeek(std::queue<ExtentIterator *> &ext_its, DataChunk& output, DataChunk &input, idx_t nodeColIdx, std::vector<duckdb::LogicalType> &scanSchema, vector<ExtentID> &target_eids, vector<idx_t> &boundary_position, idx_t current_pos, vector<idx_t> output_col_idx) {
	ExtentID target_eid = target_eids[current_pos]; // TODO make this functionality as Macro --> GetEIDFromPhysicalID
	ExtentID current_eid;
	auto ext_it = ext_its.front();
	D_ASSERT(ext_it != nullptr || ext_it->IsInitialized());
