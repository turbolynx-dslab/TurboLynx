...
StoreAPIResult iTbgppGraphStore::getAdjListFromVid(
    AdjacencyListIterator &adj_iter, int adjColIdx, ExtentID &prev_eid,
    uint64_t vid, uint64_t *&start_ptr, uint64_t *&end_ptr,
    ExpandDirection expand_dir)
{
    D_ASSERT(expand_dir == ExpandDirection::OUTGOING ||
    ...
    if (target_eid != prev_eid) {
        ...
        if (expand_dir == ExpandDirection::OUTGOING) {
            is_initialized = adj_iter.Initialize(client, adjColIdx, target_eid,
                                                 LogicalType::FORWARD_ADJLIST);
        }
        else if (expand_dir == ExpandDirection::INCOMING) {
            is_initialized = adj_iter.Initialize(client, adjColIdx, target_eid,
                                                 LogicalType::BACKWARD_ADJLIST);
        }
        adj_iter.getAdjListPtr(vid, target_eid, &start_ptr, &end_ptr,
                               is_initialized);
    }
    else {
        adj_iter.getAdjListPtr(vid, target_eid, &start_ptr, &end_ptr,
