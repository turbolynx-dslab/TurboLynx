...
PhysicalSort::PhysicalSort(Schema &sch, vector<BoundOrderByNode> orders_p):
	CypherPhysicalOperator(PhysicalOperatorType::SORT, sch), orders(std::move(orders_p)) { }
...
public:
	SortSinkState(BufferManager &buffer_manager, const PhysicalSort &order, RowLayout &payload_layout)
...
unique_ptr<LocalSinkState> PhysicalSort::GetLocalSinkState(ExecutionContext &context) const {
    // create global sort state
    RowLayout payload_layout;
	payload_layout.Initialize(types);
    auto result = make_unique<SortSinkState>(BufferManager::GetBufferManager(*context.client), *this, payload_layout);
...
SinkResultType PhysicalSort::Sink(ExecutionContext &context, DataChunk &input, LocalSinkState &lstate) const {
...
void PhysicalSort::Combine(ExecutionContext& context, LocalSinkState& lstate) const {
	auto &state = (SortSinkState &)lstate;
...
unique_ptr<LocalSourceState> PhysicalSort::GetLocalSourceState(ExecutionContext &context) const {
	return make_unique<PhysicalSortOperatorState>();
}

void PhysicalSort::GetData(ExecutionContext &context, DataChunk &chunk, LocalSourceState &lstate, LocalSinkState &sink_state) const {
...
bool PhysicalSort::IsSourceDataRemaining(LocalSourceState &lstate, LocalSinkState &sink_state) const {
	auto &state = (PhysicalSortOperatorState &)lstate;
