                          ...
                          const uint min, const DistanceFunc &disfunc)
{
    ...
    for (uint pid = 0; pid < this->_datalen; ++pid) {
        ...
        if (!this->_visited[pid]) {
            ...
            const std::vector<uint> neighbors = this->regionQuery(pid);
...
{
    // it has unvisited , visited unassigned pts. visited assigned will not appear
    ...
    while (border.size() > 0) {
        ...
        if (!this->_visited[pid]) {
            ...
            // std::cout << "pidneighbors.size() = " << pidneighbors.size() << std::endl;
