...
unique_ptr<QueryGraph> Binder::bindPatternElement(
    ...
    for (auto i = 0u; i < patternElement.getNumPatternElementChains(); ++i) {
        ...
        auto rightNode = bindQueryNode(*patternElementChain->getNodePattern(), *queryGraph, collection);
        bindQueryRel(*patternElementChain->getRelPattern(), leftNode, rightNode, *queryGraph, collection);
        leftNode = rightNode;
    }
