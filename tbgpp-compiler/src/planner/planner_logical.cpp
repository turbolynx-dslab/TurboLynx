...
LogicalPlan *Planner::lPlanQueryPart(const NormalizedQueryPart &queryPart, LogicalPlan *prev_plan)
{
...
LogicalPlan *Planner::lPlanReadingClause(BoundReadingClause *boundReadingClause, LogicalPlan *prev_plan)
{
	LogicalPlan *plan;
...
LogicalPlan *Planner::lPlanMatchClause(BoundReadingClause *boundReadingClause, LogicalPlan *prev_plan)
{
...
// {
//     LogicalPlan *plan = nullptr;
//     GPOS_ASSERT(prev_plan != nullptr);

//     CMemoryPool *mp = this->memory_pool;
// 	vector<CColRef *> outer_join_colrefs;
// 	vector<CColRef *> inner_join_colrefs;
...
//                     prev_plan->getSchema()->isNodeBound(lhs_name);
//                 is_lhs_bound = is_lhs_bound || is_lhs_bound_on_outer;
//                 is_rhs_bound_on_outer =
//                     prev_plan->getSchema()->isNodeBound(rhs_name);
...
//             if (!is_lhs_bound ||
//                 (is_lhs_bound &&
//                  !is_lhs_bound_on_outer)) {  // lhs not bound or bound on inner
//                 // Join with R
//                 if (!is_lhs_bound && !is_lhs_bound_on_outer) {
//                     lhs_plan =
//                         lPlanNodeOrRelExpr((NodeOrRelExpression *)lhs, true);
//                 }
//                 else {
...
//                 lhs_plan->addBinaryParentOp(a_r_join_expr, edge_plan);
//             }
//             else { // lhs bound on outer
//                 // No join, only filter with A
//                 lhs_plan = edge_plan;
//                 CExpression *selection_expr = CUtils::PexprLogicalSelect(
//                     mp, lhs_plan->getPlanExpr(),
//                     lExprScalarCmpEq(lExprScalarPropertyExpr(
//                                          lhs_name, ID_COLNAME, prev_plan),
//                                      lExprScalarPropertyExpr(
//                                          edge_name, SID_COLNAME, lhs_plan)));
//                 lhs_plan->addUnaryParentOp(selection_expr);
// 				outer_join_colrefs.push_back(prev_plan->getSchema()->getColRefOfKey(lhs_name, ID_COLNAME));
// 				inner_join_colrefs.push_back(lhs_plan->getSchema()->getColRefOfKey(edge_name, SID_COLNAME));
//             }
//             D_ASSERT(lhs_plan != nullptr);

//             // Plan R - B
//             if (is_lhs_bound && is_rhs_bound) {
// 				if (is_lhs_bound_on_outer && !is_rhs_bound_on_outer) {
//                     // join case
//                     rhs_plan = qg_plan;
//                     auto join_expr = lExprLogicalJoin(
//                         lhs_plan->getPlanExpr(), rhs_plan->getPlanExpr(),
...
// 										right_pred));
// 					hop_plan->addUnaryParentOp(selection_expr);
// 					if (is_rhs_bound_on_outer) {
// 						outer_join_colrefs.push_back(prev_plan->getSchema()->getColRefOfKey(rhs_name, ID_COLNAME));
// 						inner_join_colrefs.push_back(lhs_plan->getSchema()->getColRefOfKey(edge_name, TID_COLNAME));
// 					}
// 				}
//             }
...
//             if (prev_plan->getSchema()->isNodeBound(node_name)) {
//                 D_ASSERT(
...
//     qg_plan->getPlanExpr()->OsPrint(oss);
//     GPOS_TRACE(str.GetBuffer());

//     GPOS_ASSERT(outer_join_colrefs.size() != 0);
//     GPOS_ASSERT(inner_join_colrefs.size() != 0);

//     CExpression *lo_join_expr;
//     CExpression *pexprEquality;
//     if (outer_join_colrefs.size() == 1 && inner_join_colrefs.size() == 1) {
//         // single predicate
//         pexprEquality = CUtils::PexprScalarEqCmp(mp, outer_join_colrefs[0],
//                                                  inner_join_colrefs[0]);
//     }
//     else {
//         // multiple predicates
//         GPOS_ASSERT(outer_join_colrefs.size() == inner_join_colrefs.size());
//         CExpressionArray *pdrgpexprChildren = GPOS_NEW(mp) CExpressionArray(mp);
//         for (auto i = 0; i < outer_join_colrefs.size(); i++) {
//             pdrgpexprChildren->Append(CUtils::PexprScalarEqCmp(
//                 mp, outer_join_colrefs[i], inner_join_colrefs[i]));
//         }
//         pexprEquality = GPOS_NEW(mp) CExpression(
//             mp, GPOS_NEW(mp) CScalarBoolOp(mp, CScalarBoolOp::EboolopAnd),
//             pdrgpexprChildren);
//     }

//     lo_join_expr = CUtils::PexprLogicalApply<CLogicalLeftOuterApply>(
//         mp, prev_plan->getPlanExpr(), qg_plan->getPlanExpr(), pexprEquality);

//     prev_plan->getSchema()->appendSchema(qg_plan->getSchema());
//     prev_plan->addBinaryParentOp(lo_join_expr, qg_plan);
...
LogicalPlan *Planner::lPlanNodeOrRelExpr(NodeOrRelExpression *node_expr, bool is_node) {
	...
	if (node_expr->isDSITarget()) { // do dynamic schema instantiation // TODO we need DSI trigger logic
		...
		for (auto &t_oid: representative_table_oids) {
			schema_proj_mapping.insert({t_oid, map<uint64_t, uint64_t>()});
		}
		GPOS_ASSERT(schema_proj_mapping.size() == representative_table_oids.size());
			...
			for (auto &t_oid: representative_table_oids) {
				...
				if (true) {
					...
					schema_proj_mapping.find(t_oid)->
						second.insert({(uint64_t)tmp++, (uint64_t)col_idx});
				} else {
    ...
    get_output = std::move(lExprLogicalGetNodeOrEdge(
        node_name_print, table_oids, &schema_proj_mapping,
        table_oids.size() == 1 ? false : true));
    get_output = std::move(lExprLogicalGetNodeOrEdge(
...
std::pair<CExpression*, CColRefArray*> Planner::lExprLogicalGetNodeOrEdge(string name, vector<uint64_t> &relation_oids, CColRef2dArray *prev_input_array,
	CColRefArray *prev_output_array, map<uint64_t, map<uint64_t, uint64_t>> *schema_proj_mapping, bool insert_projection) {
	...
	CColRef2dArray *pdrgdrgpcrInput;
	CExpressionArray *pdrgpexpr;
	// idx0_output_array = GPOS_NEW(mp) CColRefArray(mp);
	// idx0_output_array->AppendArray(prev_output_array);
	if (relation_oids.size() > 1) {
		// pdrgpcrOutput = GPOS_NEW(mp) CColRefArray(mp);
		pdrgdrgpcrInput = GPOS_NEW(mp) CColRef2dArray(mp);
		pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
	}
	// pdrgdrgpcrInput->AppendArray(prev_input_array);
	for (int idx = 0; idx < relation_oids.size(); idx++) {
		auto& oid = relation_oids[idx];
		...
		if (relation_oids.size() == 1) {
			...
			if (idx == 0) {
				idx0_output_array = output_array;
				pdrgdrgpcrInput->Append(output_array);
				// pdrgdrgpcrInput->Append(prev_output_array);
			} else {
				pdrgdrgpcrInput->Append(output_array);
			}
			pdrgpexpr->Append(expr);
	...
	vector<vector<uint64_t>> &table_oids_in_groups, map<uint64_t, map<uint64_t, uint64_t>> *schema_proj_mapping, bool insert_projection)
{
	...
	if (do_schema_mapping) {
		for (int col_idx = 0; col_idx < num_proj_cols; col_idx++) {
			...
			valid_cid = col_idx; // TODO temp bug
			// GPOS_ASSERT(valid_cid != std::numeric_limits<uint64_t>::max());
			// extract info and maintain vector of column type infos
			if (valid_cid != std::numeric_limits<uint64_t>::max()) {
				auto *mdcol = lGetRelMd(valid_oid)->GetMdCol(valid_cid);
				gpmd::IMDId *col_type_imdid = mdcol->MdidType();
				gpos::INT col_type_modifier = mdcol->TypeModifier();
				union_schema_types.push_back(make_pair(col_type_imdid, col_type_modifier));
			} else {
				uint32_t null_type_id = LOGICAL_TYPE_BASE_ID + (OID)duckdb::LogicalTypeId::SQLNULL;
				CMDIdGPDB *col_type_imdid = GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, null_type_id, 1, 0);
				gpos::INT col_type_modifier = 0;
				union_schema_types.push_back(make_pair(col_type_imdid, col_type_modifier));
			}
		}
	}
