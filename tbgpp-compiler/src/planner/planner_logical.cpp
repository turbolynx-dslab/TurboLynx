                                         ...
                                         bool is_node)
{
    ...
    if (node_expr
            ->isDSITarget()) {  // do dynamic schema instantiation // TODO we need DSI trigger logic
        ...
        context->db->GetCatalogWrapper().ConvertTableOidsIntoRepresentativeOids(
            *context, prop_key_ids, table_oids, representative_table_oids,
            table_oids_in_groups);

        for (auto &t_oid : representative_table_oids) {
            schema_proj_mapping.insert({t_oid, map<uint64_t, uint64_t>()});
        }
        GPOS_ASSERT(schema_proj_mapping.size() ==
                    representative_table_oids.size());
...
std::pair<CExpression *, CColRefArray *> Planner::lExprLogicalGetNodeOrEdge(
    string name, vector<uint64_t> &relation_oids,
    CColRef2dArray *prev_input_array, CColRefArray *prev_output_array,
    map<uint64_t, map<uint64_t, uint64_t>> *schema_proj_mapping,
    bool insert_projection)
{
...
Planner::lExprLogicalGetNodeOrEdgeForDSI(
    string name, vector<uint64_t> &relation_oids,
    map<uint64_t, map<uint64_t, uint64_t>> *schema_proj_mapping,
    bool insert_projection)
{
...
std::pair<CExpression *, CColRefArray *> Planner::lExprLogicalGetNodeOrEdge(
    string name, vector<uint64_t> &relation_oids,
    vector<vector<uint64_t>> &table_oids_in_groups,
    map<uint64_t, map<uint64_t, uint64_t>> *schema_proj_mapping,
    bool insert_projection)
{
    CMemoryPool *mp = this->memory_pool;
    ...
    if (do_schema_mapping) {
        for (int col_idx = 0; col_idx < num_proj_cols; col_idx++) {
            ...
            if (valid_cid != std::numeric_limits<uint64_t>::max()) {
                ...
                uint32_t null_type_id =
                    LOGICAL_TYPE_BASE_ID + (OID)duckdb::LogicalTypeId::SQLNULL;
                CMDIdGPDB *col_type_imdid = GPOS_NEW(mp)
                    CMDIdGPDB(IMDId::EmdidGeneral, null_type_id, 1, 0);
...
std::pair<CExpression *, CColRefArray *> Planner::lExprLogicalGetNodeOrEdge(
    string name, uint64_t instance_oid,
    map<uint64_t, map<uint64_t, uint64_t>> *schema_proj_mapping,
    bool insert_projection)
{
