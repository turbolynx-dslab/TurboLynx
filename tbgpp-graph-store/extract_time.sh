...
build_index=`(grep -nr -e 'Index Build' $file | awk '{s+=$4}END{print s}')`

total_elapsed=`(grep -nr -e 'Load' $file | grep -e 'Done!' | grep -e 'Elapsed' | awk '{s+=$6}END{print s}')`

echo "INIT_CSV" $init_csv "READ_CSV" $read_csv "CREATE_EXT" $create_ext, "BUILD_MAP" $build_map, "BUILD_INDEX" $build_index, "TOTAL" $total_elapsed
