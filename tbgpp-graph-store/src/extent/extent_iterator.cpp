...
void ExtentIterator::Rewind() {
    ...
    is_rewinded = true;
    current_eid = (ExtentID)std::numeric_limits<uint32_t>::max();
    bool cache_sucess = ObtainFromCache(ext_ids_to_iterate[current_idx], toggle);
    if (!cache_sucess) throw NotImplementedException("Rewind: Cache miss");
}
...
bool ExtentIterator::GetNextExtent(ClientContext &context, DataChunk &output, ExtentID &output_eid, size_t scan_size, bool is_output_chunk_initialized) {
    ...
    for (idx_t i = 0; i < ext_property_type.size(); i++) output_column_idxs.push_back(i);
    idx_t scan_begin_offset = 0, scan_end_offset = 0;
    getScanRange(scan_size, scan_begin_offset, scan_end_offset);
    referenceRows(output, output_eid, scan_size, output_column_idxs, scan_begin_offset, scan_end_offset);
...
void ExtentIterator::requestIOForDoubleBuffering(ClientContext &context) {
    ...
    if (support_double_buffering && current_idx < max_idx && current_idx_in_this_extent == 0) {
        ...
        if (current_idx < max_idx - 1) {
            if (!ObtainFromCache(ext_ids_to_iterate[current_idx + 1], next_toggle)) {
                ExtentCatalogEntry* extent_cat_entry = 
...
void ExtentIterator::referenceRows(DataChunk &output, ExtentID output_eid, size_t scan_size, vector<idx_t> &output_column_idxs, idx_t scan_begin_offset, idx_t scan_end_offset) {
    CompressionHeader comp_header;
    ...
    for (size_t i = 0; i < ext_property_type.size(); i++) {
        ...
        if (ext_property_type[i] != LogicalType::ID) {
            ...
            if (comp_header.HasNullMask()) {
                ...
                size_t bitmap_ptr_offset = comp_header.GetNullBitmapOffset() + (scan_begin_offset / 8);
                validity = ValidityMask((uint64_t *)(io_requested_buf_ptrs[toggle][i] + bitmap_ptr_offset));
            }
        }
...
#endif
                                for (auto prefetch_idx = 0; prefetch_idx < prefetch_unit_size && seqno_idx + prefetch_idx < target_seqnos.size(); prefetch_idx++) {
                                    if (src_validity.RowIsValid(src_data_seqnos[seqno_idx + prefetch_idx])) {
                                        strings[target_seqnos[seqno_idx + prefetch_idx]] = varchar_arr[src_data_seqnos[seqno_idx + prefetch_idx]];
                                    } else {
                                        validity.SetInvalid(target_seqnos[seqno_idx + prefetch_idx]);
                                    }
                                }
                            }
