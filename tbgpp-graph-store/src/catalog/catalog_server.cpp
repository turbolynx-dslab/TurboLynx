...
CatalogServer::CatalogServer(const std::string &unix_socket, std::string shm_directory)
  ...
  catalog_segment = new fixed_managed_mapped_file(boost::interprocess::open_or_create, shm_path.c_str(), 2 * 1024 * 1024 * 1024UL, (void *) CATALOG_ADDR);
...
bool CatalogServer::recreate() {
  ...
  // Create shared memory
  catalog_segment = new fixed_managed_mapped_file(boost::interprocess::create_only, shm_path.c_str(), 2 * 1024 * 1024 * 1024UL, (void *) CATALOG_ADDR);
