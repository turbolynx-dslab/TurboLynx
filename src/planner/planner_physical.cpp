...
static idx_t GetActiveTailColumnCount(
    duckdb::CypherPhysicalOperatorGroups *groups)
{
    auto *tail = GetActiveTailOperator(groups);
    return tail ? tail->GetTypes().size() : idx_t(0);
}

static void ConstructOffsetColMapping(idx_t input_size, idx_t output_offset,
                                      vector<uint32_t> &out_mapping)
{
    out_mapping.assign(input_size, std::numeric_limits<uint32_t>::max());
    for (idx_t i = 0; i < input_size; i++) {
        out_mapping[i] = static_cast<uint32_t>(output_offset + i);
    }
}

static idx_t FindExactCurIdOnlyIndex(CColRefArray *cols, const CColRef *target)
{
    if (!cols || !target) {
        return gpos::ulong_max;
    }
    for (ULONG i = 0; i < cols->Size(); i++) {
        if (CColRef::EqualsCurIdOnly((*cols)[i], target)) {
            return i;
        }
    }
    return gpos::ulong_max;
...
duckdb::idx_t Planner::pFindColRefIndex(CColRefArray *cols,
                                        const CColRef *target,
                                        const std::vector<bool> *skip_claimed)
{
    if (!cols || !target) {
        return gpos::ulong_max;
    }
    auto canonical_name = [&](const CColRef *col) {
        if (!col) {
            return std::string();
        }
        std::wstring ws(col->Name().Pstr()->GetBuffer());
        auto name = std::string(ws.begin(), ws.end());
        auto dot = name.rfind('.');
        return dot == std::string::npos ? name : name.substr(dot + 1);
    };
    auto is_hidden_internal = [&](const CColRef *col) {
        if (!col) {
            return false;
        }
        auto *table_col = dynamic_cast<const CColRefTable *>(col);
        if (table_col && table_col->AttrNum() < 0) {
            return true;
        }
        auto name = canonical_name(col);
        return !name.empty() && name[0] == '_';
    };
    auto is_skipped = [&](ULONG idx) {
        return skip_claimed && idx < skip_claimed->size() &&
               (*skip_claimed)[idx];
    };

    auto find_exact = [&](CColRefArray *haystack,
                          const CColRef *needle) -> duckdb::idx_t {
        for (ULONG i = 0; i < haystack->Size(); i++) {
            if (is_skipped(i)) {
                continue;
            }
            if (CColRef::EqualsCurIdOnly((*haystack)[i], needle)) {
                return i;
            }
        }
        return gpos::ulong_max;
    };
    auto find_by_lineage = [&](CColRefArray *haystack,
                               const CColRef *needle) -> duckdb::idx_t {
        ...
        auto target_lineage = collect_lineage(needle);
        duckdb::idx_t match = gpos::ulong_max;
        for (ULONG i = 0; i < haystack->Size(); i++) {
            if (is_skipped(i)) {
                continue;
            }
            auto *candidate = (*haystack)[i];
            if (!candidate) {
                continue;
            }
    ...
    auto find_by_node_prop = [&](CColRefArray *haystack,
                                 const CColRef *needle) -> duckdb::idx_t {
        const auto target_node_id = needle->NodeId();
        const auto target_prop_id = pGetColumnPropertyKeyId(needle);
        ...
        for (ULONG i = 0; i < haystack->Size(); i++) {
            ...
            if (candidate->NodeId() == target_node_id &&
                pGetColumnPropertyKeyId(candidate) == target_prop_id) {
                return i;
            }
        }
        return gpos::ulong_max;
    ...
    auto find_by_node_name = [&](CColRefArray *haystack,
                                 const CColRef *needle) -> duckdb::idx_t {
        const auto target_node_id = needle->NodeId();
        if (target_node_id == gpos::ulong_max) {
            return gpos::ulong_max;
    ...
    auto find_by_unique_name = [&](CColRefArray *haystack,
                                   const CColRef *needle) -> duckdb::idx_t {
        auto *target_name = needle->Name().Pstr()->GetBuffer();
        duckdb::idx_t match = gpos::ulong_max;
        for (ULONG i = 0; i < haystack->Size(); i++) {
    ...
    auto idx = find_exact(cols, target);
    if (idx != gpos::ulong_max) {
        return idx;
    }
    idx = find_by_lineage(cols, target);
    if (idx != gpos::ulong_max) {
        return idx;
    }
    ...
    idx = find_by_node_prop(cols, target);
    if (idx != gpos::ulong_max) {
        return idx;
    }
    ...
    idx = find_by_node_name(cols, target);
    if (idx != gpos::ulong_max) {
        return idx;
    }
    return find_by_unique_name(cols, target);
}

CColRefArray *Planner::pGetTrackedPhysicalOutputCols(
    CMemoryPool *mp, duckdb::idx_t actual_output_col_count)
{
    if (physical_plan_output_colrefs.empty() ||
        physical_plan_output_positions.size() !=
            physical_plan_output_colrefs.size() ||
        actual_output_col_count == 0) {
        return nullptr;
    }

    auto *cols = GPOS_NEW(mp) CColRefArray(mp);
    std::vector<CColRef *> ordered(actual_output_col_count, nullptr);
    bool fits_positions = true;
    for (idx_t i = 0; i < physical_plan_output_colrefs.size(); i++) {
        auto pos = physical_plan_output_positions[i];
        if (pos >= actual_output_col_count || ordered[pos] != nullptr) {
            fits_positions = false;
            break;
        }
        ordered[pos] = physical_plan_output_colrefs[i];
    }
    if (fits_positions) {
        for (idx_t pos = 0; pos < actual_output_col_count; pos++) {
            if (!ordered[pos]) {
                return nullptr;
            }
            cols->Append(ordered[pos]);
        }
        return cols;
    }

    if (actual_output_col_count != physical_plan_output_colrefs.size()) {
        return nullptr;
    }
    for (auto *col : physical_plan_output_colrefs) {
        cols->Append(col);
    }
    return cols;
}

CColRefArray *Planner::pGetCurrentPhysicalOutputCols(
...
CColRefArray *Planner::pConcatColRefArrays(CMemoryPool *mp,
                                           CColRefArray *lhs_cols,
                                           CColRefArray *rhs_cols)
{
    auto *out = GPOS_NEW(mp) CColRefArray(mp);
    if (lhs_cols) {
        out->AppendArray(lhs_cols);
    }
    if (rhs_cols) {
        out->AppendArray(rhs_cols);
    }
    return out;
}
...
void Planner::pSetExplicitPhysicalOutputLayout(CColRefArray *cols)
{
    ...
    for (ULONG idx = 0; idx < cols->Size(); idx++) {
        physical_plan_output_colrefs.push_back(cols->operator[](idx));
...
Planner::pTraverseTransformPhysicalPlan(CExpression *plan_expr)
{
	...
	*/
    ...
    auto actual_output_col_count = GetActiveTailColumnCount(result);
    CColRefArray *derived_output_cols = plan_expr->DeriveOutputColumns()->Pdrgpcr(mp);
    bool preserve_custom_output_cols =
        plan_expr->Pop()->Eopid() ==
            COperator::EOperatorId::EopPhysicalSerialUnionAll &&
        pIsUnionAllOpAccessExpression(plan_expr);
    bool preserve_explicit_output_cols =
        preserve_explicit_physical_output_layout &&
        physical_plan_output_positions.size() ==
            physical_plan_output_colrefs.size() &&
        physical_plan_output_colrefs.size() == actual_output_col_count;
    bool prefer_derived_output_cols =
...
duckdb::CypherPhysicalOperatorGroups *Planner::pTransformEopDSITableScan(CExpression *plan_expr) {
    ...
    if (is_filter_exist) {
        for (ULONG i = 0; i < output_cols->Size(); i++) {
            auto idx = scan_cols->IndexOf((*output_cols)[i]);
            D_ASSERT(idx != gpos::ulong_max);
            bound_ref_idxs.push_back((duckdb::idx_t)idx);
        }
        is_filter_only_column_exist = output_cols->Size() < scan_cols->Size();
    }
    ...
    if (is_filter_only_column_exist) {
        ...
        pGetColumnsDuckDBType(output_cols, proj_types);
...
Planner::pTransformEopUnionAllForNodeOrEdgeScan(CExpression *plan_expr)
{
    ...
    auto *union_op = (CPhysicalUnionAll *)plan_expr->Pop();
    auto *union_output_cols = union_op->PdrgpcrOutput();
    auto *union_input_cols = union_op->PdrgpdrgpcrInput();
    vector<CColRef *> representative_output_cols;
    representative_output_cols.reserve(union_output_cols->Size());
    for (ULONG col_idx = 0; col_idx < union_output_cols->Size(); col_idx++) {
        CColRef *representative = union_output_cols->operator[](col_idx);
        for (ULONG child_idx = 0; child_idx < union_input_cols->Size(); child_idx++) {
            auto *child_cols = union_input_cols->operator[](child_idx);
            if (col_idx >= child_cols->Size()) {
                continue;
            }
            CColRef *candidate = child_cols->operator[](col_idx);
            std::wstring ws(candidate->Name().Pstr()->GetBuffer());
        ...
        representative_output_cols.push_back(representative);
    }
    auto set_actual_output_colrefs = [&](const vector<duckdb::idx_t> *keep_idxs) {
        physical_plan_output_colrefs.clear();
        physical_plan_output_positions.clear();
        if (keep_idxs == nullptr) {
            for (duckdb::idx_t idx = 0; idx < representative_output_cols.size();
                 idx++) {
                auto *col = representative_output_cols[idx];
                physical_plan_output_colrefs.push_back(col);
                physical_plan_output_positions.push_back(idx);
            }
            return;
        }
        if (representative_output_cols.size() == keep_idxs->size()) {
            for (duckdb::idx_t idx = 0; idx < representative_output_cols.size();
                 idx++) {
                auto *col = representative_output_cols[idx];
                physical_plan_output_colrefs.push_back(col);
        ...
        for (auto idx : *keep_idxs) {
            D_ASSERT(idx < representative_output_cols.size());
            physical_plan_output_colrefs.push_back(
                representative_output_cols[idx]);
            physical_plan_output_positions.push_back(idx);
        }
    };
    if (has_filter) {
        ...
        */
        ...
        CColRefArray *final_output_cols =
        ...
        if (has_filter_only_column) {
            ...
            if (unionall_output_cols->Size() ==
                non_filter_only_column_idxs.size()) {
                pGetColumnsDuckDBType(unionall_output_cols,
                                     proj_op_output_types);
            } else {
                auto input_types = global_schema.getStoredTypes();
                for (auto idx : non_filter_only_column_idxs) {
                    D_ASSERT(idx < input_types.size());
                    proj_op_output_types.push_back(input_types[idx]);
                }
            }
            ...
            set_actual_output_colrefs(&non_filter_only_column_idxs);
        } else {
            set_actual_output_colrefs(nullptr);
        }
    }
    else {
    ...
    if (!is_dsi_table) {
        ...
        for (int i = 0; i < proj_list_expr->PdrgPexpr()->Size(); i++) {
            ...
            if (pMatchExprPattern(proj_elem_expr, scalarident_pattern)) {
                ...
                col_prop_ids.push_back(
                    pGetColumnPropertyKeyId(ident_op->Pcr()));
                if (global_types[i] == duckdb::LogicalType::SQLNULL) {
                    global_types[i] = duckdb_type;
                }
            }
    ...
    CExpression *plan_expr, bool is_left_outer)
{
    ...
    */
    ...
    auto *idxscan_op = (CPhysicalIndexScan *)idxscan_expr->Pop();
    OID idxscan_table_oid =
        CMDIdGPDB::CastMdid(idxscan_op->Ptabdesc()->MDId())->Oid();

    auto resolve_col_table_oid = [&](const CColRef *col, OID &table_oid) -> bool {
        auto *table_mdid = col ? col->GetMdidTable() : nullptr;
        if (!IMDId::IsValid(table_mdid)) {
            return false;
        }
        table_oid = CMDIdGPDB::CastMdid(table_mdid)->Oid();
        return true;
    };
    auto has_non_idxscan_property_col = [&](CColRefArray *cols) -> bool {
        for (ULONG col_idx = 0; col_idx < cols->Size(); col_idx++) {
            auto *col = cols->operator[](col_idx);
            if (!pIsColEdgeProperty(col)) {
                continue;
            }
            OID table_oid;
            if (!resolve_col_table_oid(col, table_oid)) {
                continue;
            }
            if (table_oid != idxscan_table_oid) {
                return true;
            }
        }
        return false;
    };
    auto filter_has_non_idxscan_property_col =
        [&](CExpression *root_expr) -> bool {
        auto *filter_expr = pFindFilterExpr(root_expr);
        if (filter_expr == NULL) {
            return false;
        }

        vector<CExpression *> nodes_to_visit;
        nodes_to_visit.push_back(filter_expr->operator[](1));
        while (!nodes_to_visit.empty()) {
            CExpression *current_node = nodes_to_visit.back();
            nodes_to_visit.pop_back();
            if (current_node->Pop()->Eopid() ==
                COperator::EOperatorId::EopScalarIdent) {
                auto *ident = (CScalarIdent *)current_node->Pop();
                auto *col = ident->Pcr();
                if (!pIsColEdgeProperty(col)) {
                    continue;
                }
            ...
            for (ULONG child_idx = 0; child_idx < current_node->Arity();
                 child_idx++) {
                nodes_to_visit.push_back(current_node->operator[](child_idx));
            }
        }
        return false;
    };

    if (has_non_idxscan_property_col(inner_cols) ||
        filter_has_non_idxscan_property_col(plan_expr->operator[](1))) {
    ...
    CColRefArray *actual_outer_cols = pGetCurrentPhysicalOutputCols(
        mp, outer_cols, GetActiveTailColumnCount(result));
    ...
    size_t num_outer_schemas = GetActiveTailOperator(result)
    ...
    auto find_outer_physical_idx_pred = [&](CExpression *pred) -> duckdb::idx_t {
        ...
        for (ULONG ci = 0; ci < pred->Arity(); ci++) {
            ...
            if (c->Pop()->Eopid() == COperator::EOperatorId::EopScalarIdent) {
                ...
                auto idx = pFindColRefIndex(actual_outer_cols, ident->Pcr());
    ...
    edge_physical_id_col = pGetIDColInCols(
        ((CPhysicalIndexScan *)idxscan_expr->Pop())->PdrgpcrOutput());
    D_ASSERT(edge_physical_id_col != NULL);
    ...
    **/
    ...
    if (!generate_seek) {
        ...
        pConstructColMapping(adj_inner_cols, adj_output_cols,
                             inner_col_maps_adj[0]);
        edge_id_col_idx = FindExactCurIdOnlyIndex(adj_output_cols,
                                                  edge_physical_id_col);
        if (edge_id_col_idx == gpos::ulong_max) {
            edge_id_col_idx =
                pFindColRefIndex(adj_output_cols, edge_physical_id_col);
        }
        std::string adj_cols_dbg;
        for (ULONG i = 0; i < adj_output_cols->Size(); i++) {
            auto *col = adj_output_cols->operator[](i);
            if (i) {
                adj_cols_dbg += " | ";
            }
            std::wstring ws(col->Name().Pstr()->GetBuffer());
            std::string name(ws.begin(), ws.end());
            adj_cols_dbg += std::to_string(i) + ":" + name + "#" +
                            std::to_string(col->Id());
        }
        std::wstring edge_ws(edge_physical_id_col->Name().Pstr()->GetBuffer());
        std::string edge_name(edge_ws.begin(), edge_ws.end());
        spdlog::debug("[ADJ-SEEK-IDX] edge_col={}#{} edge_id_col_idx={} adj_output_cols=[{}]",
                     edge_name, edge_physical_id_col->Id(), edge_id_col_idx,
                     adj_cols_dbg);
    }
    output_cols_copy->Release();
    // D_ASSERT(inner_col_maps_adj[0].size() > 0); // release this condition

    // Runtime AdjIdxJoin indexes outer inputs by the child chunk layout, not
    // ORCA's logical required set.
    pConstructColMapping(actual_outer_cols, adj_output_cols,
                         outer_col_maps_adj[0]);
    ...
    if (generate_seek) {
        ...
        D_ASSERT(edge_inner_idx < inner_col_maps_adj[0].size());
        edge_id_col_idx = inner_col_maps_adj[0][edge_inner_idx];
    ...
    */
    ...
    // Construct IdSeek
    if (generate_seek) {
        // Construct union-level inner col map first; per-schema inner col maps
        // are derived from this once we know every concrete edge graphlet that
        // can appear under the adj join.
        vector<uint32_t> union_inner_col_map_seek;
        pConstructColMapping(seek_inner_cols, seek_output_cols,
                             union_inner_col_map_seek);
        D_ASSERT(union_inner_col_map_seek.size() > 0);
        ...
        // Collect all concrete edge graphlets that can back this seek. The old
        // code only kept the first graphlet OID, which breaks UNION edge types
        // such as LIKES(Comment|Post): post edge IDs were later materialized
        // with the comment property schema and returned garbage timestamps.
        auto append_seek_oid = [&](uint64_t oid) {
            if (std::find(seek_obj_ids.begin(), seek_obj_ids.end(), oid) ==
                seek_obj_ids.end()) {
                seek_obj_ids.push_back(oid);
            }
        };
        auto append_seek_target = [&](uint64_t oid) {
            auto *entry =
                context->db->GetCatalog().GetEntry(*context, DEFAULT_SCHEMA,
                                                   oid, true);
            if (!entry) {
                return;
            }
            if (dynamic_cast<duckdb::PropertySchemaCatalogEntry *>(entry)) {
                append_seek_oid(oid);
                return;
            }
            if (auto *part =
                    dynamic_cast<duckdb::PartitionCatalogEntry *>(entry)) {
                auto *ps_oids = part->GetPropertySchemaIDs();
                if (!ps_oids) {
                    return;
                }
                for (auto ps_oid : *ps_oids) {
                    append_seek_oid(ps_oid);
                }
            }
        };
        CTableDescriptor *idx_tab_desc = idxscan_op->Ptabdesc();
        if (idx_tab_desc->IsInstanceDescriptor()) {
            IMdIdArray *table_ids = idx_tab_desc->GetTableIdsInGroup();
            for (ULONG i = 0; i < table_ids->Size(); i++) {
                auto *mdid = CMDIdGPDB::CastMdid((*table_ids)[i]);
                append_seek_target(mdid->Oid());
            }
        } else {
            auto *tab_mdid = CMDIdGPDB::CastMdid(idx_tab_desc->MDId());
            duckdb::idx_t graphlet_oid = tab_mdid->Oid();
            append_seek_target(graphlet_oid);
            auto it = multi_edge_partitions.find(graphlet_oid);
            if (it != multi_edge_partitions.end()) {
                for (auto sib_oid : it->second) {
                    append_seek_target(sib_oid);
                }
            }
        }
        D_ASSERT(!seek_obj_ids.empty());

        vector<duckdb::LogicalType> seek_scan_global_types;
        vector<duckdb::idx_t> seek_scan_prop_ids;
        pGetColumnsDuckDBType(seek_inner_cols, seek_scan_global_types,
                              seek_scan_prop_ids);
        {
            std::string seek_cols_dbg;
            for (ULONG i = 0; i < seek_inner_cols->Size(); i++) {
                auto *col = seek_inner_cols->operator[](i);
                auto *tbl_col = dynamic_cast<CColRefTable *>(col);
                if (i) {
                    seek_cols_dbg += " | ";
                }
                std::wstring ws(col->Name().Pstr()->GetBuffer());
                std::string name(ws.begin(), ws.end());
                seek_cols_dbg += name + "#" + std::to_string(col->Id());
                if (tbl_col) {
                    seek_cols_dbg += "(attr=" +
                                     std::to_string(tbl_col->AttrNum()) +
                                     ",prop=" +
                                     std::to_string(tbl_col->PropId()) + ")";
                }
            }
            std::string seek_prop_ids_dbg;
            for (idx_t i = 0; i < seek_scan_prop_ids.size(); i++) {
                if (i) {
                    seek_prop_ids_dbg += ",";
                }
                seek_prop_ids_dbg += std::to_string(seek_scan_prop_ids[i]);
            }
            spdlog::debug(
                "[SEEK-INNER-COLS] edge_id_col_idx={} cols=[{}] prop_ids=[{}]",
                edge_id_col_idx, seek_cols_dbg, seek_prop_ids_dbg);
        }

        inner_col_maps_seek.assign(seek_obj_ids.size(),
                                   union_inner_col_map_seek);
        scan_projection_mappings_seek.assign(seek_obj_ids.size(), {});
        output_projection_mappings_seek.assign(seek_obj_ids.size(), {});
        scan_types_seek.assign(seek_obj_ids.size(), {});

        auto &cat_instance = context->db->GetCatalog();
        for (idx_t schema_idx = 0; schema_idx < seek_obj_ids.size();
             schema_idx++) {
            auto *ps_cat =
                (duckdb::PropertySchemaCatalogEntry *)cat_instance.GetEntry(
                    *context, DEFAULT_SCHEMA, seek_obj_ids[schema_idx], true);
            D_ASSERT(ps_cat != nullptr);
            duckdb::PropertyKeyID_vector *key_ids = ps_cat->GetKeyIDs();
            {
                std::string key_ids_dbg;
                for (idx_t ki = 0; ki < key_ids->size(); ki++) {
                    if (ki) {
                        key_ids_dbg += ",";
                    }
                    key_ids_dbg += std::to_string((*key_ids)[ki]);
                }
                spdlog::debug("[SEEK-KEYS] oid={} schema_idx={} key_ids=[{}]",
                             seek_obj_ids[schema_idx], schema_idx,
                             key_ids_dbg);
            }
            pGenerateMappingInfo(seek_scan_prop_ids, key_ids,
                                 seek_scan_global_types,
                                 scan_types_seek[schema_idx],
                                 output_projection_mappings_seek[schema_idx],
                                 scan_projection_mappings_seek[schema_idx]);
        }
        ...
        if (!filter_in_seek) {
            ...
            // Build one logical filter once, then clone it for each concrete
            // edge schema. The seek-side column layout is identical across
            // schemas because missing properties are represented as SQLNULL
            // placeholders in scan_types_seek / scan_projection_mappings_seek.
            vector<unique_ptr<duckdb::Expression>> base_filter_exprs;
            pGetFilterDuckDBExprs(filter_expr, adj_output_cols, seek_inner_cols,
                                  adj_output_cols->Size(),
                                  base_filter_exprs);
            vector<duckdb::idx_t> base_filter_col_idxs;
            if (!base_filter_exprs.empty()) {
                pGetIdentIndices(base_filter_exprs[0], base_filter_col_idxs);
            }
            vector<vector<unique_ptr<duckdb::Expression>>> filter_duckdb_exprs(
                seek_obj_ids.size());
            vector<vector<duckdb::idx_t>> seek_filter_col_idxs(
                seek_obj_ids.size(), base_filter_col_idxs);
            for (idx_t schema_idx = 0; schema_idx < seek_obj_ids.size();
                 schema_idx++) {
                for (auto &expr : base_filter_exprs) {
                    filter_duckdb_exprs[schema_idx].push_back(
            ...
            duckdb_idseek_op = new duckdb::PhysicalIdSeek(
                ...
                outer_col_maps_seek, inner_col_maps_seek,
                union_inner_col_map_seek, scan_projection_mappings_seek,
                scan_types_seek, filter_duckdb_exprs, seek_filter_col_idxs,
    ...
    CExpression *plan_expr)
{
    ...
    for (ULONG col_idx = 0; col_idx < actual_outer_cols->Size(); col_idx++) {
        CColRef *col = actual_outer_cols->operator[](col_idx);
        ULONG col_id = col->Id();
        // find key column
...
void Planner::pExpandVirtualPartitionForIdSeek(
    ...
    std::vector<std::vector<std::unique_ptr<duckdb::Expression>>> *per_schema_filter_exprs,
    std::vector<std::vector<duckdb::idx_t>> *per_schema_filter_col_idxs)
{
    ...
    for (size_t si = sib_start; si < vp_it->second.size(); si++) {
        ...
        if (per_schema_filter_exprs) {
            D_ASSERT(part_idx < per_schema_filter_exprs->size());
            std::vector<std::unique_ptr<duckdb::Expression>> copied_exprs;
            copied_exprs.reserve((*per_schema_filter_exprs)[part_idx].size());
            for (auto &expr : (*per_schema_filter_exprs)[part_idx]) {
                copied_exprs.push_back(expr ? expr->Copy() : nullptr);
            }
            per_schema_filter_exprs->push_back(std::move(copied_exprs));
        }
        if (per_schema_filter_col_idxs) {
            D_ASSERT(part_idx < per_schema_filter_col_idxs->size());
            per_schema_filter_col_idxs->push_back(
                (*per_schema_filter_col_idxs)[part_idx]);
        }
    }
}
...
Planner::pTransformEopPhysicalInnerIndexNLJoinToIdSeekNormal(CExpression *plan_expr)
{
    ...
    while (true) {
        if (inner_root->Pop()->Eopid() ==
            ...
            if (is_complex_comparison) {
                ...
                */
                ...
                for (ULONG col_idx = 0; col_idx < org_filter_pred_expr->Arity();
                    ...
                    for (ULONG child_col_idx = 0;
                        ...
                        if (idxscan_cols->IndexOf(col) == gpos::ulong_max) {
                            auto col_idx =
                                pFindColRefIndex(actual_outer_cols, col);
                ...
                for (ULONG i = 0; i < outer_filter_cols_idx.size() - 1; i++) {
                    CColRef *lpcr =
                        actual_outer_cols->operator[](outer_filter_cols_idx[i]);
                    CColRef *rpcr =
                        actual_outer_cols->operator[](outer_filter_cols_idx[i + 1]);
                ...
                filter_duckdb_expr =
                    pTransformScalarExpr(filter_pred_expr, actual_outer_cols,
            ...
            for (ULONG col_idx = 0; col_idx < output_cols->Size(); col_idx++) {
                CColRef *col = (*output_cols)[col_idx];
                ULONG idx = pFindColRefIndex(actual_outer_cols, col);
                if (idx ==
                    gpos::
                    ...
                    if (!project_physical_id_column) {
                        ...
                        for (ULONG outer_col_idx = 0;
                             outer_col_idx < actual_outer_cols->Size();
                             outer_col_idx++) {
                            CColRef *col = (*actual_outer_cols)[outer_col_idx];
            ...
            */
            ...
            filter_duckdb_expr = pTransformScalarExpr(
                filter_pred_expr, actual_outer_cols, inner_required_cols);
            ...
            if (scan_types_before_filter != inner_cols->Size()) {
                ...
                size_t threshold =
                    actual_outer_cols->Size() + inner_cols->Size();
            ...
            outer_col_map.reserve(actual_outer_cols->Size());
            for (ULONG col_idx = 0; col_idx < actual_outer_cols->Size();
                    col_idx++) {
                CColRef *col = actual_outer_cols->operator[](col_idx);
                ULONG col_id = col->Id();
    ...
    pExpandVirtualPartitionForIdSeek(
        ...
        inner_col_maps, output_projection_mapping, &per_schema_filter_exprs,
        &filter_col_idxs);
    ...
    CExpression *plan_expr)
{
    ...
    bool rhs_pid_only = pIsJoinRhsOutputPhysicalIdOnly(plan_expr);
    ...
    while (true) {
        if (inner_root->Pop()->Eopid() ==
            ...
            for (uint32_t i = 0; i < inner_root->Arity();
                ...
                if (i == 0) {
                    for (ULONG col_idx = 0;
                         col_idx < actual_outer_cols->Size();
                        col_idx++) {
                        CColRef *col = actual_outer_cols->operator[](col_idx);
                        ULONG col_id = col->Id();
                        // match _tid
        ...
        duckdb::CypherPhysicalOperatorGroups *result)
{
    ...
    // Construct outer_col_map
    for (int outer_col_idx = 0; outer_col_idx < actual_outer_cols->Size();
         outer_col_idx++) {
        CColRef *col = actual_outer_cols->operator[](outer_col_idx);
        auto col_idx = pFindColRefIndex(seek_output_cols, col);
    ...
    if (repr_proj_list_expr->Arity() == 1) {
        ...
        if (pIsPhysicalIdCol(proj_col)) {
            ...
            schema_proj.setStoredTypes(output_types_proj);
            pGetProjectionExprs(actual_outer_cols, seek_output_cols,
                                output_types_proj, proj_exprs);
            if (proj_exprs.size() != 0) {
                duckdb::CypherPhysicalOperator *duckdb_proj_op =
                    new duckdb::PhysicalProjection(schema_proj,
        ...
        if (i == REPR_IDX) {
            ...
            if (pIsComplexCondition(seek_condition)) {
                ...
                CExpression *condition_filter_expr =  CUtils::PexprScalarCmp(
                        ...
                        IMDType::ECmpType::EcmptEq);
                pGetFilterDuckDBExprs(condition_filter_expr, actual_outer_cols,
                                      nullptr, 0,
                                      condition_filter_duckdb_exprs);
                pFindOperandsColIdxs(seek_condition->operator[](0),
                                     actual_outer_cols, sid_col_idx);
            }
            else {
                pFindOperandsColIdxs(seek_condition, actual_outer_cols,
                                     sid_col_idx);
            }
        ...
        if (is_dsi_table) {
            ...
            for (int j = 0; j < proj_list_expr->Arity(); j++) {
                ...
                if (scalar_expr->Pop()->Eopid() == COperator::EOperatorId::EopScalarIdent) {
                    col_prop_ids.push_back(
                        pGetColumnPropertyKeyId(proj_elem->Pcr()));
    ...
    if (seperate_filter_from_condition) {
        ...
        pGetDuckDBTypesFromColRefs(actual_outer_cols,
                                   output_types_condition_filter);
...
Planner::pTransformEopPhysicalInnerIndexNLJoinToIdSeekDSI(CExpression *plan_expr)
{
            ...
            COperator::EOperatorId::EopPhysicalComputeScalarColumnar)
        {
            ...
            if (seek_condition->Pop()->Eopid() == COperator::EOperatorId::EopScalarBoolOp) {
                ...
                CExpression *cycle_filter_expr =  CUtils::PexprScalarCmp(
                    ...
                    seek_condition->operator[](1)->operator[](1),
                    IMDType::ECmpType::EcmptEq);
                pGetFilterDuckDBExprs(cycle_filter_expr, actual_outer_cols,
                                      nullptr, 0, cycle_filter_duckdb_exprs);
            }
                ...
                for (uint32_t j = 0; j < projectlist_expr->Arity(); j++) {
                    ...
                    CScalarProjectElement *proj_elem =
                        (CScalarProjectElement
                             *)(projectlist_expr->operator[](j)->Pop());
                    CColRefTable *proj_col = (CColRefTable *)proj_elem->Pcr();
                    ULONG col_prop_id = pGetColumnPropertyKeyId(proj_col);
                    bool found = false;
    ...
    if (skip_seek) {
        ...
        pGetDuckDBTypesFromColRefs(output_cols, output_types_proj);
        schema_proj.setStoredTypes(output_types_proj);
        pGetProjectionExprsWithJoinCond(scalar_cmp_expr, actual_outer_cols,
                                        output_cols, output_types_proj,
                                        proj_exprs);
        if (proj_exprs.size() != 0) {
            duckdb::CypherPhysicalOperator *duckdb_proj_op =
                new duckdb::PhysicalProjection(schema_proj,
    ...
    CExpression *plan_expr, duckdb::CypherPhysicalOperatorGroups *result)
{
    ...
    CExpression *cmp_expr = nullptr;
    vector<COperator::EOperatorId> union_inner_pattern = {
        COperator::EOperatorId::EopPhysicalSerialUnionAll,
        COperator::EOperatorId::EopPhysicalComputeScalarColumnar,
        COperator::EOperatorId::EopPhysicalIndexScan};
    if (pMatchExprPattern(pexprInner, union_inner_pattern, 0, true)) {
        auto *compute_scalar_expr = pexprInner->operator[](0);
        auto *index_scan_expr = compute_scalar_expr->operator[](0);
        cmp_expr = index_scan_expr->operator[](0);
    }

    vector<unique_ptr<duckdb::Expression>> proj_exprs;
    if (cmp_expr != nullptr) {
        pGetProjectionExprsWithJoinCond(cmp_expr, actual_outer_cols, output_cols,
                                        proj_output_types, proj_exprs);
    } else {
        pGetProjectionExprs(actual_outer_cols, output_cols, proj_output_types,
                            proj_exprs);
    }
...
Planner::pTransformEopPhysicalHashJoinToHashJoin(CExpression *plan_expr)
{
    ...
    duckdb::CypherPhysicalOperatorGroups *rhs_result =
        pTraverseTransformPhysicalPlan(pexprRight);
    auto rhs_sfg_state = pCaptureSchemaFlowGraphState();
    CColRefArray *actual_right_cols = pGetCurrentPhysicalOutputCols(
        mp, right_cols, GetActiveTailColumnCount(rhs_result));
    duckdb::CypherPhysicalOperatorGroups *lhs_result =
        pTraverseTransformPhysicalPlan(pexprLeft);
    auto lhs_sfg_state = pCaptureSchemaFlowGraphState();
    CColRefArray *actual_left_cols = pGetCurrentPhysicalOutputCols(
    ...
    pConstructColMapping(actual_left_cols, hash_output_cols, left_col_map);
    pConstructColMapping(actual_right_cols, hash_output_cols, right_col_map);
    ...
    if (join_type != duckdb::JoinType::ANTI &&
        ...
        // JoinHashTable consumes RHS payload columns in the same order as the
        // non-null entries of output_right_projection_map. If we derive the
        // build payload from an unordered set difference, the runtime starts
        // gathering the wrong RHS column into each output slot.
        D_ASSERT(actual_right_cols->Size() == right_col_map.size());
        for (ULONG right_idx = 0; right_idx < actual_right_cols->Size();
             right_idx++) {
            if (right_col_map[right_idx] ==
                std::numeric_limits<uint32_t>::max()) {
                continue;
            }
            right_build_map.push_back(right_idx);
            auto *right_col = actual_right_cols->operator[](right_idx);
            OID type_oid =
                CMDIdGPDB::CastMdid(right_col->RetrieveType()->MDId())->Oid();
            INT type_mod = right_col->TypeModifier();
    ...
    return pBuildSchemaflowGraphForBinaryJoin(plan_expr, op, schema,
                                              ...
                                              std::move(lhs_sfg_state),
                                              std::move(rhs_sfg_state),
                                              is_right_outer);
}
    ...
    CExpression *plan_expr)
{
    ...
    duckdb::CypherPhysicalOperatorGroups *rhs_result =
        pTraverseTransformPhysicalPlan(pexprRight);
    auto rhs_sfg_state = pCaptureSchemaFlowGraphState();
    // Capture RHS physical output cols immediately after RHS traversal —
    // pGetCurrentPhysicalOutputCols reads the current tracking state, which is
    // overwritten when the LHS subtree is traversed. Deferring this call until
    // after the LHS traversal yields LHS cols here, collapsing both sides to
    // the same ColRef ids and breaking downstream ScalarIdent resolution.
    CColRefArray *actual_right_cols = pGetCurrentPhysicalOutputCols(
        mp, right_cols, GetActiveTailColumnCount(rhs_result));
    ...
    CColRefArray *actual_left_cols = pGetCurrentPhysicalOutputCols(
        mp, left_cols, GetActiveTailColumnCount(lhs_result));
    CColRefArray *physical_output_cols =
        pConcatColRefArrays(mp, actual_left_cols, actual_right_cols);

    vector<duckdb::LogicalType> types;
    vector<uint32_t> left_col_map;
    vector<uint32_t> right_col_map;

    pGetDuckDBTypesFromColRefs(physical_output_cols, types);
    // CrossProduct materializes output as [left..., right...]. Using lineage-
    // based matching here breaks self-joins because the two sides can share
    // ColRef ancestry and swap slots.
    ConstructOffsetColMapping(actual_left_cols->Size(), 0, left_col_map);
    ConstructOffsetColMapping(actual_right_cols->Size(),
                              actual_left_cols->Size(), right_col_map);
                                                      ...
                                                      bool is_correlated)
{
    ...
    duckdb::CypherPhysicalOperatorGroups *rhs_result =
        pTraverseTransformPhysicalPlan(pexprInner);
    auto rhs_sfg_state = pCaptureSchemaFlowGraphState();
    CColRefArray *actual_inner_cols = pGetCurrentPhysicalOutputCols(
        mp, inner_cols, GetActiveTailColumnCount(rhs_result));
    duckdb::CypherPhysicalOperatorGroups *lhs_result =
        pTraverseTransformPhysicalPlan(pexprOuter);
    auto lhs_sfg_state = pCaptureSchemaFlowGraphState();
    CColRefArray *actual_outer_cols = pGetCurrentPhysicalOutputCols(
        mp, outer_cols, GetActiveTailColumnCount(lhs_result));
    ...
    if (join_type == duckdb::JoinType::SEMI ||
        ...
        physical_output_cols = actual_outer_cols;
    }
    else {
        physical_output_cols =
            pConcatColRefArrays(mp, actual_outer_cols, actual_inner_cols);
    }

    pGetDuckDBTypesFromColRefs(physical_output_cols, types);
    // BlockwiseNLJoin also materializes output as [outer..., inner...].
    // Self-joins can make lineage-based matching ambiguous across the two
    // sides, so construct the positional mapping directly.
    ConstructOffsetColMapping(actual_outer_cols->Size(), 0, outer_col_map);
    if (join_type == duckdb::JoinType::SEMI ||
        join_type == duckdb::JoinType::ANTI) {
        inner_col_map.assign(actual_inner_cols->Size(),
                             std::numeric_limits<uint32_t>::max());
    }
    else {
        ConstructOffsetColMapping(actual_inner_cols->Size(),
                                  actual_outer_cols->Size(), inner_col_map);
    }
    ...
    unique_ptr<duckdb::Expression> join_condition_expr = pTransformScalarExpr(
        join_pred, actual_outer_cols, actual_inner_cols);
...
Planner::pTransformEopProjectionColumnar(CExpression *plan_expr)
{
    ...
    CColRefArray *physical_child_cols = GPOS_NEW(mp) CColRefArray(mp);
    if (!physical_plan_output_colrefs.empty()) {
        for (auto *colref : physical_plan_output_colrefs) {
            physical_child_cols->Append(colref);
        }
    } else {
        auto *derived_child_cols =
            pexprProjRelational->DeriveOutputColumns()->Pdrgpcr(mp);
        for (ULONG i = 0; i < derived_child_cols->Size(); i++) {
            physical_child_cols->Append(derived_child_cols->operator[](i));
        }
    }
    vector<duckdb::idx_t> physical_child_positions;
    if (!physical_plan_output_colrefs.empty() &&
        physical_plan_output_positions.size() ==
            physical_plan_output_colrefs.size()) {
        if (child_cols->Size() == 0 &&
            actual_child_col_count > physical_plan_output_colrefs.size()) {
            auto trailing_base = actual_child_col_count -
                                 physical_plan_output_colrefs.size();
            physical_child_positions.reserve(physical_plan_output_colrefs.size());
            for (duckdb::idx_t idx = 0;
                 idx < physical_plan_output_colrefs.size(); idx++) {
                physical_child_positions.push_back(trailing_base + idx);
            }
        } else {
            physical_child_positions = physical_plan_output_positions;
        }
    } else {
        physical_child_positions.reserve(physical_child_cols->Size());
        if (child_cols->Size() == 0 &&
            actual_child_col_count > physical_child_cols->Size()) {
            auto trailing_base = actual_child_col_count -
                                 physical_child_cols->Size();
            for (duckdb::idx_t idx = 0; idx < physical_child_cols->Size(); idx++) {
                physical_child_positions.push_back(trailing_base + idx);
            }
        } else {
            for (duckdb::idx_t idx = 0; idx < physical_child_cols->Size(); idx++) {
                physical_child_positions.push_back(idx);
            }
        }
    }
    auto canonical_col_name = [](const CColRef *col) {
        std::wstring ws(col->Name().Pstr()->GetBuffer());
        auto name = std::string(ws.begin(), ws.end());
        auto dot = name.rfind('.');
        return dot == std::string::npos ? name : name.substr(dot + 1);
    };
    auto find_physical_child_idx = [&](const CColRef *target_col) -> ULONG {
        for (size_t pi = 0; pi < physical_plan_output_colrefs.size(); pi++) {
            if (physical_plan_output_colrefs[pi]->Id() == target_col->Id()) {
                return static_cast<ULONG>(physical_child_positions[pi]);
            }
        }
        auto target_name = canonical_col_name(target_col);
        for (size_t pi = 0; pi < physical_plan_output_colrefs.size(); pi++) {
            if (canonical_col_name(physical_plan_output_colrefs[pi]) ==
                target_name) {
                return static_cast<ULONG>(physical_child_positions[pi]);
            }
        }
        ULONG child_idx = child_cols->IndexOf(const_cast<CColRef *>(target_col));
        if (child_idx != gpos::ulong_max) {
            return child_idx;
        }
        for (ULONG i = 0; i < child_cols->Size(); i++) {
            if (canonical_col_name((*child_cols)[i]) == target_name) {
                return i;
            }
        }
        return gpos::ulong_max;
    ...
    for (size_t proj_i = 0; proj_i < indices_to_project.size(); proj_i++) {
        ...
        if (mpv_it != mpv_colref_to_scan_idx_.end()) {
            ...
            auto proj_expr =
                pTransformScalarExpr(pexprScalarExpr, physical_child_cols);
    ...
    CExpression *plan_expr)
{
    ...
    CColRefArray *child_cols =
        pexprProjRelational->Prpp()->PcrsRequired()->Pdrgpcr(mp);
    CColRefArray *physical_child_cols =
        pGetCurrentPhysicalOutputCols(mp, child_cols, actual_child_col_count);
    CColRefArray *interm_output_cols =
        plan_expr->DeriveOutputColumns()->Pdrgpcr(mp);
    CExpression *pexprProjList = (*plan_expr)[1];  // Projection list
    const CColRefArray *grouping_cols = agg_op->PdrgpcrGroupingCols();
    ...
    vector<uint32_t> child_to_physical_map;
    pConstructColMapping(child_cols, physical_child_cols,
                         child_to_physical_map);
    auto find_physical_child_idx = [&](const CColRef *target_col) -> ULONG {
        auto logical_child_idx = pFindColRefIndex(child_cols, target_col);
        if (logical_child_idx != gpos::ulong_max &&
            logical_child_idx < child_to_physical_map.size()) {
            auto mapped_idx = child_to_physical_map[logical_child_idx];
            if (mapped_idx != std::numeric_limits<uint32_t>::max()) {
                return mapped_idx;
            }
        }
        auto child_idx = pFindColRefIndex(physical_child_cols, target_col);
        if (child_idx != gpos::ulong_max) {
            return child_idx;
        }
        auto target_name = canonical_col_name(target_col);
        for (ULONG i = 0; i < physical_child_cols->Size(); i++) {
            if (canonical_col_name((*physical_child_cols)[i]) == target_name) {
                return i;
            }
        }
        child_idx = pFindColRefIndex(child_cols, target_col);
        if (child_idx != gpos::ulong_max) {
            return child_idx;
        }
        for (ULONG i = 0; i < child_cols->Size(); i++) {
        ...
        if (cols) {
            for (ULONG i = 0; i < cols->Size(); i++) {
                ...
                result += std::to_string(i) + "=" + std::to_string(col->Id()) +
                          "/" + std::to_string(col->PrevId()) + "/" +
                          std::to_string(col->NodeId()) + "/" +
                          std::to_string(col->PropId()) + ":" +
    ...
    auto describe_expr = [](CExpression *expr) {
        if (!expr || !expr->Pop()) {
            return std::string("<null>");
        }
        return std::string(expr->Pop()->SzId());
    };

    std::unordered_map<ULONG, uint64_t> grouping_output_positions;
    uint64_t num_outputs_in_grouping_col = 0;
    for (ULONG output_col_idx = 0; output_col_idx < output_cols->Size();
         output_col_idx++) {
        CColRef *col = output_cols->operator[](output_col_idx);
        ...
        grouping_output_positions[col->Id()] = num_outputs_in_grouping_col++;
    }
    ...
    for (ULONG group_col_idx = 0; group_col_idx < grouping_cols_sorted->Size();
        ...
        ULONG child_idx = find_physical_child_idx(col);
        ...
        proj_types.push_back(col_type);
        output_column_names_proj.push_back(pGetColNameFromColRef(col));
        auto output_pos_it = grouping_output_positions.find(col->Id());
        if (output_pos_it != grouping_output_positions.end()) {
            output_projection_mapping.push_back(output_pos_it->second);
        } else {
        ...
        if (pexprScalarExpr->Pop()->Eopid() != COperator::EopScalarAggFunc) {
            ...
            if (aggargs_expr->operator[](0)->Pop()->Eopid() !=
                ...
                proj_exprs.push_back(std::move(pTransformScalarExpr(
                    aggargs_expr->operator[](0), physical_child_cols)));
                agg_exprs.push_back(std::move(pTransformScalarAggFunc(
                    pexprScalarExpr, physical_child_cols, proj_exprs.back()->return_type,
                    proj_exprs.size() - 1)));
                ...
                if (has_pre_projection) {
                    agg_exprs.push_back(std::move(
                        pTransformScalarAggFunc(pexprScalarExpr, physical_child_cols,
    ...
    if (agg_exprs.empty() && agg_groups.empty() && has_post_projection) {
        ...
        for (ULONG i = 0; i < pexprProjList->Arity(); i++) {
            ...
            auto duckdb_expr = pTransformScalarExpr(se, physical_child_cols);
    ...
    CExpression *plan_expr)
{
    ...
    CExpression *pexprOuter = (*plan_expr)[0];
    CColRefArray *outer_cols = pexprOuter->Prpp()->PcrsRequired()->Pdrgpcr(mp);
    CColRefArray *derived_outer_cols =
        pexprOuter->DeriveOutputColumns()->Pdrgpcr(mp);

    CColRefArray *physical_child_cols =
        actual_child_col_count > outer_cols->Size() && derived_outer_cols &&
                derived_outer_cols->Size() == actual_child_col_count
            ? derived_outer_cols
    ...
    auto find_physical_child_idx = [&](const CColRef *target_col) -> ULONG {
        auto hidden_leading_cols =
            actual_child_col_count > physical_plan_output_colrefs.size()
                ? actual_child_col_count - physical_plan_output_colrefs.size()
                : 0;
        for (size_t pi = 0; pi < physical_plan_output_colrefs.size(); pi++) {
            if (physical_plan_output_colrefs[pi]->Id() == target_col->Id()) {
                return static_cast<ULONG>(pi + hidden_leading_cols);
            }
        }
        auto target_name = canonical_col_name(target_col);
        for (size_t pi = 0; pi < physical_plan_output_colrefs.size(); pi++) {
        ...
        if (derived_outer_cols != nullptr) {
            ULONG derived_idx =
                derived_outer_cols->IndexOf(const_cast<CColRef *>(target_col));
            if (derived_idx != gpos::ulong_max) {
                return derived_idx;
            }
            for (ULONG i = 0; i < derived_outer_cols->Size(); i++) {
                if (canonical_col_name((*derived_outer_cols)[i]) ==
                    target_name) {
                    return i;
                }
            }
        }
        ULONG outer_idx = outer_cols->IndexOf(const_cast<CColRef *>(target_col));
        if (outer_idx != gpos::ulong_max) {
            return outer_idx;
        }
        for (ULONG i = 0; i < outer_cols->Size(); i++) {
            if (canonical_col_name((*outer_cols)[i]) == target_name) {
                return i;
            }
        }
        return gpos::ulong_max;
    ...
    auto has_path_col = [&](CColRefArray *cols) {
        if (!cols) {
            return false;
        }
        for (ULONG i = 0; i < cols->Size(); i++) {
            auto name = canonical_col_name((*cols)[i]);
            if (name == "path" || name == "paths" || name == "p") {
                return true;
            }
        }
        return false;
    ...
    filter_exprs.push_back(std::move(
        pTransformScalarExpr(filter_pred_expr, physical_child_cols, nullptr)));
    if (has_path_col(output_cols) || has_path_col(outer_cols) ||
        has_path_col(physical_child_cols)) {
        spdlog::debug(
            "[FilterPath] actual_child_col_count={} output_cols={} outer_cols={} derived_outer_cols={} physical_child_cols={} pred={}",
            actual_child_col_count, describe_cols(output_cols),
            describe_cols(outer_cols), describe_cols(derived_outer_cols),
            describe_cols(physical_child_cols),
            filter_exprs.empty() ? std::string("<none>")
                                 : filter_exprs[0]->ToString());
    }
    ...
    if (output_cols->Size() != outer_cols->Size()) {
        ...
        for (ULONG col_idx = 0; col_idx < output_cols->Size(); col_idx++) {
            CColRef *col = (*output_cols)[col_idx];
            ULONG idx = find_physical_child_idx(col);
            D_ASSERT(idx != gpos::ulong_max);
            proj_exprs.push_back(make_unique<duckdb::BoundReferenceExpression>(
...
duckdb::CypherPhysicalOperatorGroups* Planner::pTransformEopAllShortestPath(CExpression* plan_expr) {
    ...
    std::wstring src_ws(pcr_src->Name().Pstr()->GetBuffer());
    std::wstring dst_ws(pcr_dest->Name().Pstr()->GetBuffer());
    std::wstring path_ws(path_col->Name().Pstr()->GetBuffer());
    spdlog::debug(
        "[AllShortestPathPlan] input_cols={} output_cols={} pcr_src={}/{}:{} pcr_dest={}/{}:{} path={}/{}:{}",
        describe_cols(input_cols), describe_cols(output_cols), pcr_src->Id(),
        pcr_src->NodeId(), std::string(src_ws.begin(), src_ws.end()),
        pcr_dest->Id(), pcr_dest->NodeId(),
        std::string(dst_ws.begin(), dst_ws.end()), path_col->Id(),
        path_col->NodeId(), std::string(path_ws.begin(), path_ws.end()));
    for (idx_t i = 0; i < input_col_map.size(); i++) {
        spdlog::debug("[AllShortestPathPlan] input_col_map[{}]={}", i,
                     input_col_map[i]);
    }
    pGetDuckDBTypesFromColRefs(output_cols, types);
    schema.setStoredTypes(types);
...
{
    spdlog::debug("[SFG-DEBUG-SingleScan] output_schema.types.size={} restrict={}",
                 output_schema.getStoredTypes().size(), restrict_generate_sfg_for_unionall);
...
{
    spdlog::info("[SFG-DEBUG-MultiScan] global_schema.types.size={} local_schemas.size={} restrict={}",
                 global_schema.getStoredTypes().size(), local_schemas.size(), restrict_generate_sfg_for_unionall);
    for (auto i = 0; i < local_schemas.size(); i++) {
        spdlog::info("[SFG-DEBUG-MultiScan]   local_schema[{}].types.size={}",
                     i, local_schemas[i].getStoredTypes().size());
    }
    if (!restrict_generate_sfg_for_unionall) {
        generate_sfg = true;
        pipeline_operator_types.push_back(duckdb::OperatorType::UNARY);
...
void Planner::pRemapBoundRefIndices(
    unique_ptr<duckdb::Expression> &expr,
    const std::vector<duckdb::idx_t> &index_map)
{
    switch (expr->expression_class) {
        case duckdb::ExpressionClass::BOUND_REF: {
            auto *ref = (duckdb::BoundReferenceExpression *)expr.get();
            if (ref->index < index_map.size()) {
                ref->index = index_map[ref->index];
            }
            break;
        }
        case duckdb::ExpressionClass::BOUND_COMPARISON: {
            auto *cmp = (duckdb::BoundComparisonExpression *)expr.get();
            pRemapBoundRefIndices(cmp->left, index_map);
            pRemapBoundRefIndices(cmp->right, index_map);
            break;
        }
        case duckdb::ExpressionClass::BOUND_CONJUNCTION: {
            auto *conj = (duckdb::BoundConjunctionExpression *)expr.get();
            for (auto &child : conj->children) {
                pRemapBoundRefIndices(child, index_map);
            }
            break;
        }
        case duckdb::ExpressionClass::BOUND_FUNCTION: {
        ...
        case duckdb::ExpressionClass::BOUND_CAST: {
            auto *cast = (duckdb::BoundCastExpression *)expr.get();
            pRemapBoundRefIndices(cast->child, index_map);
            break;
        }
        case duckdb::ExpressionClass::BOUND_OPERATOR: {
            auto *op = (duckdb::BoundOperatorExpression *)expr.get();
        ...
        case duckdb::ExpressionClass::BOUND_CASE: {
            auto *case_expr = (duckdb::BoundCaseExpression *)expr.get();
            for (auto &check : case_expr->case_checks) {
                pRemapBoundRefIndices(check.when_expr, index_map);
                pRemapBoundRefIndices(check.then_expr, index_map);
            }
            pRemapBoundRefIndices(case_expr->else_expr, index_map);
            break;
        }
        case duckdb::ExpressionClass::BOUND_BETWEEN: {
            auto *between = (duckdb::BoundBetweenExpression *)expr.get();
            pRemapBoundRefIndices(between->input, index_map);
            pRemapBoundRefIndices(between->lower, index_map);
            pRemapBoundRefIndices(between->upper, index_map);
            break;
        }
        case duckdb::ExpressionClass::BOUND_CONSTANT:
        case duckdb::ExpressionClass::BOUND_PARAMETER:
...
Planner::pBuildSchemaflowGraphForBinaryJoin(CExpression *plan_expr,
                                            ...
                                            bool swap_children)
{
    return pBuildSchemaflowGraphForBinaryJoin(plan_expr, op, output_schema,
                                              nullptr, nullptr,
                                              swap_children);
}

duckdb::CypherPhysicalOperatorGroups *
Planner::pBuildSchemaflowGraphForBinaryJoin(
    CExpression *plan_expr, duckdb::CypherPhysicalOperator *op,
    duckdb::Schema &output_schema,
    duckdb::CypherPhysicalOperatorGroups *lhs_result,
    duckdb::CypherPhysicalOperatorGroups *rhs_result, bool swap_children)
{
    /**
	...
	*/
    ...
    // Step 1. rhs pipline
    if (!rhs_result) {
        rhs_result = pTraverseTransformPhysicalPlan(
    ...
    // Step 3. lhs pipeline
    if (!lhs_result) {
        lhs_result = pTraverseTransformPhysicalPlan(
            plan_expr->PdrgPexpr()->operator[](lhs_idx));
    }
...
Planner::pBuildSchemaflowGraphForBinaryJoin(
    ...
    duckdb::CypherPhysicalOperatorGroups *lhs_result,
    duckdb::CypherPhysicalOperatorGroups *rhs_result,
    SchemaFlowGraphBuildState lhs_sfg_state,
    SchemaFlowGraphBuildState rhs_sfg_state, bool swap_children)
{
    ULONG rhs_idx = swap_children ? 0 : 1;
    ULONG lhs_idx = swap_children ? 1 : 0;
    ...
    if (!rhs_result) {
        rhs_result = pTraverseTransformPhysicalPlan(
            plan_expr->PdrgPexpr()->operator[](rhs_idx));
        rhs_sfg_state = pCaptureSchemaFlowGraphState();
    }
    else {
        pRestoreSchemaFlowGraphState(std::move(rhs_sfg_state));
    }
    rhs_result->push_back(op);
    auto pipeline = new duckdb::CypherPipeline(*rhs_result);
    pipelines.push_back(pipeline);
    ...
    if (!lhs_result) {
        lhs_result = pTraverseTransformPhysicalPlan(
            plan_expr->PdrgPexpr()->operator[](lhs_idx));
        lhs_sfg_state = pCaptureSchemaFlowGraphState();
    }
    else {
        pRestoreSchemaFlowGraphState(std::move(lhs_sfg_state));
    }
    lhs_result->push_back(op);
...
duckdb::PropertyKeyID Planner::pGetColumnPropertyKeyId(const CColRef *col)
{
    auto *col_ref_table = dynamic_cast<const CColRefTable *>(col);
    if (!col_ref_table) {
        return std::numeric_limits<duckdb::PropertyKeyID>::max();
    }
    if (col_ref_table->AttrNum() == INT(-1)) {
        return 0;
    }

    auto *graph_catalog_entry =
        (duckdb::GraphCatalogEntry *)context->db->GetCatalog().GetEntry(
            *context, duckdb::CatalogType::GRAPH_ENTRY, DEFAULT_SCHEMA,
            DEFAULT_GRAPH, true);
    if (!graph_catalog_entry) {
        return col_ref_table->PropId();
    }
                                    ...
                                    vector<duckdb::idx_t>& col_prop_ids)
{
    for (ULONG col_idx = 0; col_idx < columns->Size(); col_idx++) {
        CColRef *col = (*columns)[col_idx];
        col_prop_ids.push_back(pGetColumnPropertyKeyId(col));
        output_types.push_back(pGetColumnsDuckDBType(col));
    }
}
                                          ...
                                          CColRefArray *colrefs)
{
    ...
    for (ULONG child_idx = 0; child_idx < pred->Arity(); child_idx++) {
        ...
        if (child->Pop()->Eopid() == COperator::EOperatorId::EopScalarIdent) {
            ...
            idx = pFindColRefIndex(colrefs, ident->Pcr());
            if (idx != gpos::ulong_max) {
                break;
            }
        }
...
void Planner::pConstructColMapping(CColRefArray *in_cols,
                                   ...
                                   vector<uint32_t> &out_mapping)
{
    out_mapping.assign(in_cols->Size(), std::numeric_limits<uint32_t>::max());
    if (!in_cols || !out_cols) {
        return;
    }

    // Build an injective input->output mapping. Hidden/internal input columns can
    // share lineage with user-visible output columns, so an input-driven fuzzy
    // match can incorrectly map multiple inputs onto the same output slot
    // (e.g. [0, 0, 1, 2, ...]). Walking outputs first ensures each logical
    // output position is claimed by at most one input column, while still using
    // the planner's fallback matching rules to recover remapped colrefs.
    std::vector<bool> input_claimed(in_cols->Size(), false);
    std::vector<bool> output_claimed(out_cols->Size(), false);
    for (ULONG out_idx = 0; out_idx < out_cols->Size(); out_idx++) {
        CColRef *out_col = out_cols->operator[](out_idx);
        for (ULONG in_idx = 0; in_idx < in_cols->Size(); in_idx++) {
            if (input_claimed[in_idx]) {
                continue;
            }
            if (!CColRef::EqualsCurIdOnly(in_cols->operator[](in_idx),
                                          out_col)) {
                continue;
            }
            out_mapping[in_idx] = static_cast<uint32_t>(out_idx);
            input_claimed[in_idx] = true;
            output_claimed[out_idx] = true;
            break;
        }
    }
    for (ULONG out_idx = 0; out_idx < out_cols->Size(); out_idx++) {
        if (output_claimed[out_idx]) {
            continue;
        }
        CColRef *out_col = out_cols->operator[](out_idx);
        auto in_idx = pFindColRefIndex(in_cols, out_col, &input_claimed);
        if (in_idx == gpos::ulong_max || input_claimed[in_idx]) {
            continue;
        }
        out_mapping[in_idx] = static_cast<uint32_t>(out_idx);
        input_claimed[in_idx] = true;
    ...
    vector<duckdb::idx_t> &non_filter_only_column_idxs)
{
    ...
    auto has_matching_output_col = [&](const CColRef *proj_out,
                                       const CColRef *ident) {
        if (final_output_cols->IndexOf(proj_out) != gpos::ulong_max) {
            return true;
        }
        auto proj_out_name = canonical_name(proj_out);
        auto ident_name = canonical_name(ident);
        for (ULONG i = 0; i < final_output_cols->Size(); i++) {
            auto *candidate = (*final_output_cols)[i];
            if (candidate->Id() == proj_out->Id() ||
                candidate->Id() == ident->Id()) {
                return true;
            }
            auto candidate_name = canonical_name(candidate);
            if (candidate_name == proj_out_name ||
                candidate_name == ident_name) {
                return true;
            }
        }
        return false;
    ...
    for (int j = 0; j < proj_list_expr->PdrgPexpr()->Size(); j++) {
        ...
        if (pMatchExprPattern(proj_elem, scalarident_pattern)) {
            ...
            bool keep_output_col =
                has_matching_output_col(proj_elem_op->Pcr(), ident_op->Pcr());
            if (keep_output_col) {
...
bool Planner::pFindOperandsColIdxs(CExpression *expr, CColRefArray *cols, duckdb::idx_t &out_idx) {
    for (uint32_t i = 0; i < expr->Arity(); i++) {
        ...
        if (scalar_expr->Pop()->Eopid() == COperator::EOperatorId::EopScalarIdent) {
            ...
            // Operand binding for seek/join keys must stay exact. Allow the
            // underlying CColRef equality (current/prev id), but do not fall
            // back to name/lineage heuristics that can confuse hidden _id with
            // a visible business property such as `id`.
