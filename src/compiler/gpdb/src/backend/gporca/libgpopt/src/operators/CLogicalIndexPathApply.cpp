...
CLogicalIndexPathApply::CLogicalIndexPathApply(CMemoryPool *mp)
	: CLogicalApply(mp),
...
CLogicalIndexPathApply::~CLogicalIndexPathApply()
{
...
CMaxCard
CLogicalIndexPathApply::DeriveMaxCard(CMemoryPool *,  // mp
...
CXformSet *
CLogicalIndexPathApply::PxfsCandidates(CMemoryPool *mp) const
{
	CXformSet *xform_set = GPOS_NEW(mp) CXformSet(mp);
	(void) xform_set->ExchangeSet(CXform::ExfImplementIndexPathApply);
	return xform_set;
}

BOOL
CLogicalIndexPathApply::Matches(COperator *pop) const
{
	...
	if (pop->Eopid() == Eopid())
	{
		return m_pdrgpcrOuterRefs->Equals(
			CLogicalIndexPathApply::PopConvert(pop)->PdrgPcrOuterRefs());
	}
...
IStatistics *
CLogicalIndexPathApply::PstatsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
) const
{
	...
	IStatistics *stats = CJoinStatsProcessor::CalcAllJoinStats(
		...
		const_cast<CLogicalIndexPathApply *>(this));
...
COperator *
CLogicalIndexPathApply::PopCopyWithRemappedColumns(CMemoryPool *mp,
											   ...
											   BOOL must_exist)
{
	...
	result = GPOS_NEW(mp) CLogicalIndexPathApply(mp,
												path_join_lower_bound, path_join_upper_bound,
												colref_array, m_fOuterJoin,
