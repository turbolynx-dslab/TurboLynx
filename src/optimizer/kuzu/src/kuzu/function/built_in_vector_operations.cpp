...
#include "kuzu/function//comparison/vector_comparison_operations.h"
#include "kuzu/function//date/vector_date_operations.h"
// #include "kuzu/function//interval/vector_interval_operations.h"
#include "kuzu/function//list/vector_list_operations.h"
#include "kuzu/function//string/vector_string_operations.h"
// #include "kuzu/function//timestamp/vector_timestamp_operations.h"
...
bool BuiltInVectorOperations::canApplyStaticEvaluation(
    ...
    if ((functionName == CAST_TO_DATE_FUNC_NAME || functionName == CAST_TO_TIMESTAMP_FUNC_NAME ||
        ...
        children[0]->expressionType == LITERAL && children[0]->dataType.typeID == DataTypeID::STRING) {
        return true; // bind as literal
    }
    if (functionName == CAST_TO_YEAR_FUNC_NAME && 
        children[0]->expressionType == LITERAL && children[0]->dataType.typeID == DataTypeID::DATE) {
        return true; // bind as literal
    }
    return false;
}
...
VectorOperationDefinition* BuiltInVectorOperations::getBestMatch(
    ...
    for (auto& function : functions) {
        ...
        for (auto& parameterTypeID : function->parameterTypeIDs) {
            if (distinctParameterTypes.find(parameterTypeID) == distinctParameterTypes.end()) {
                // not found
...
uint32_t BuiltInVectorOperations::castRules(DataTypeID inputTypeID, DataTypeID targetTypeID) {
    if (inputTypeID == DataTypeID::ANY) {
        // DataTypeID::ANY type can be any type
        return 0;
    }
    if (targetTypeID == DataTypeID::ANY) {
        // Any inputTypeID can match to type DataTypeID::ANY
        return 0;
    }
...
void BuiltInVectorOperations::registerArithmeticOperations() {
    ...
    // vectorOperations.insert({ASIN_FUNC_NAME, AsinVecttgtttorOperation::getDefinitions()});
...
//         ID_FUNC_NAME, vector<DataTypeID>{DataTypeID::NODE}, DataTypeID::NODE_ID, nullptr));
//     definitions.push_back(make_unique<VectorOperationDefinition>(
//         ID_FUNC_NAME, vector<DataTypeID>{DataTypeID::REL}, DataTypeID::INT64, nullptr));
