...
string ExceptionFormatValue::Format(const string &msg, vector<ExceptionFormatValue> &values) {
	...
	for (auto &val : values) {
		...
		case ExceptionFormatValueType::FORMAT_VALUE_TYPE_DOUBLE:
			format_args.push_back(fmt::internal::make_arg<fmt::printf_context>(val.dbl_val));
			break;
		case ExceptionFormatValueType::FORMAT_VALUE_TYPE_INTEGER:
			format_args.push_back(fmt::internal::make_arg<fmt::printf_context>(val.int_val));
			break;
		case ExceptionFormatValueType::FORMAT_VALUE_TYPE_STRING:
			format_args.push_back(fmt::internal::make_arg<fmt::printf_context>(val.str_val));
			break;
		}
	}
	return fmt::vsprintf(msg, fmt::basic_format_args<fmt::printf_context>(
