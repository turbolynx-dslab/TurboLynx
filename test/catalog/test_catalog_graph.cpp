...
// =============================================================================
// Multi-graph — independence of multiple graphs
// =============================================================================

TEST_CASE("Graph: two graphs have disjoint OIDs for all entries",
          "[catalog][graph][multi]") {
    TestDB db;
    VertexSchema vsA = build_vertex_schema(db.ctx(), db.catalog(),
                                           "graph_A", "User",
                                           {"email"}, {LogicalType::VARCHAR});
    VertexSchema vsB = build_vertex_schema(db.ctx(), db.catalog(),
                                           "graph_B", "Item",
                                           {"title"}, {LogicalType::VARCHAR});

    // Graph OIDs are distinct
    REQUIRE(vsA.graph_oid != vsB.graph_oid);
    // Partition OIDs are distinct
    REQUIRE(vsA.partition_oid != vsB.partition_oid);
    // PS OIDs are distinct
    REQUIRE(vsA.ps_oid != vsB.ps_oid);
    // Extent OIDs are distinct
    REQUIRE(vsA.extent_oid != vsB.extent_oid);
}

TEST_CASE("Graph: two graphs have independent propertykey_maps",
          "[catalog][graph][multi]") {
    TestDB db;
    // Both graphs use the same property name "score"
    build_vertex_schema(db.ctx(), db.catalog(),
                        "graph_X", "Player",
                        {"score"}, {LogicalType::INTEGER});
    build_vertex_schema(db.ctx(), db.catalog(),
                        "graph_Y", "Item",
                        {"score"}, {LogicalType::DOUBLE});

    auto *gX = db.catalog().GetEntry<GraphCatalogEntry>(
        db.ctx(), DEFAULT_SCHEMA, "graph_X");
    auto *gY = db.catalog().GetEntry<GraphCatalogEntry>(
        db.ctx(), DEFAULT_SCHEMA, "graph_Y");

    // Each graph has "score" in its own propertykey_map
    REQUIRE(gX->propertykey_map.count("score") == 1);
    REQUIRE(gY->propertykey_map.count("score") == 1);

    // PropertyKeyID counters are per-graph (both start at 0), so both graphs
    // assign ID 0 to their first property "score". Equal numeric values are
    // expected — independence is verified by the no-bleed checks below.
    REQUIRE(gX->propertykey_map.count("score") == 1);
    REQUIRE(gY->propertykey_map.count("score") == 1);

    // Labels do not bleed across graphs
    REQUIRE(gX->vertexlabel_map.count("Item")   == 0);
    REQUIRE(gY->vertexlabel_map.count("Player") == 0);
}
