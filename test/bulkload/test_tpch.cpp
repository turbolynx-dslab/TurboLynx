...
TEST_CASE("Bulkload TPC-H SF1", "[bulkload][tpch][sf1]") {
    const auto& cfg = bulktest::DatasetRegistry::get("tpch-sf1");

    auto data_dir = bulktest::DatasetLocator::find(cfg, g_settings.data_dir,
                                                    g_settings.do_download);
    if (!data_dir) {
        WARN("tpch-sf1 data not found at: " + (g_settings.data_dir / cfg.local_path).string()
             + "  -- set --data-dir to the directory containing tpch/sf1/");
        g_skip_requested = true;
        return;
    }

    INFO("Running bulkload for tpch-sf1 from: " << data_dir->string());
