...
//  Stage 2 : REGION+NATION + nation_isLocatedIn fwd (25 edges)
//  Stage 3 : REGION+NATION + nation_isLocatedIn fwd+bwd (backward derived from fwd)
//  Stage 4 : All TPC-H vertices, no edges           (~8M rows)
//  Stage 5 : All TPC-H vertices + small edges only
//  Stage 6 : Full TPC-H SF1                         (all edges)
          ...
          "[bulkload][staged][stage6]")
{
    ...
    auto add_edge = [&](const char* type, const char* fwd) {
        bulktest::EdgeConfig e;
        e.type = type;
        e.fwd_files = {fwd};
        cfg.edges.push_back(std::move(e));
    ...
    add_edge("CUST_BELONG_TO", "customer_belongTo_nation.tbl");
    add_edge("COMPOSED_BY",    "lineitem_composedBy_part.tbl");
    add_edge("IS_PART_OF",     "lineitem_isPartOf_orders.tbl");
    add_edge("SUPPLIED_BY",    "lineitem_suppliedBy_supplier.tbl");
    add_edge("IS_LOCATED_IN",  "nation_isLocatedIn_region.tbl");
    add_edge("MADE_BY",        "orders_madeBy_customer.tbl");
    add_edge("SUPP_BELONG_TO", "supplier_belongTo_nation.tbl");
    add_edge("PARTSUPP",       "partsupp.tbl");
