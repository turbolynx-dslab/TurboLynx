...
for query_num in $queries; do
    ...
    output_str=$(timeout 3600s ../../build-release/tbgpp-client/TurboGraph-S62 --workspace:${database_path} --query:"$query_str" --disable-merge-join --compile-only --num-iterations:4 --join-order-optimizer:exhaustive --warmup)
    exit_status=$?
