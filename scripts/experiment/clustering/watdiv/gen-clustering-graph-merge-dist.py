...
def calculate_geomean_speedups(data, mode):
    df = pd.DataFrame(data, columns=['Query', 'Algorithm', 'Measure', 'Layering', 'TotalTime'])
    ...
    if mode == 'measure':
        ...
        speedups = defaultdict(list)  # Directly use a single dictionary for grouping
        ...
        for layering in layerings:
            ...
            for query in baseline_times.index:
                if query in layering_times.index:
                    ...
                    speedups[layering].append(speedup)
                    print(f"Query: {query}, Baseline: {baseline_times[query]}, Layering: {layering_times[query]}, Speedup: {speedup}")
        x_labels = layerings
        y_values = [gmean(speedups[layering]) for layering in layerings]

    # Print per-query speedups in tabular format
    print("\nPer-query Speedups:")
    ...
    for key in speedups:
        row = [key]
        row.extend([f"{speedup:.2f}" for speedup in speedups[key]])
