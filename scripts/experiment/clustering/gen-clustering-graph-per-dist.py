        ...
        for measure in measures:
            ...
            for query in baseline_times.index:
                if query in measure_times.index:
                    ...
                    print(f"Query: {query}, Baseline: {baseline_times[query]}, Measure: {measure_times[query]}, Speedup: {speedup}")
        x_labels = measures
        ...
        for algorithm in algorithms:
            ...
            for query in baseline_times.index:
                if query in algorithm_times.index:
                    ...
                    print(f"Query: {query}, Baseline: {baseline_times[query]}, Algorithm: {algorithm_times[query]}, Speedup: {speedup}")
        x_labels = algorithms
        ...
        for layering in layerings:
            ...
            for query in baseline_times.index:
                if query in layering_times.index:
                    ...
                    print(f"Query: {query}, Baseline: {baseline_times[query]}, Layering: {layering_times[query]}, Speedup: {speedup}")
        x_labels = layerings
        y_values = [gmean([speedup for query in speedups for speedup in speedups[query][layering]]) for layering in layerings]

    return x_labels, y_values

def plot_bar_chart(x_labels, y_values, mode, output_file, custom_labels, figsize, sf_number, title_fontsize, label_fontsize, tick_fontsize, distribution):
    ...
    plt.title(f'Geometric Mean Speedup by {mode.capitalize()} - Distribution {distribution}', fontsize=title_fontsize)
...
def main(folder, mode, sf_number):
    ...
    for distribution in distributions:
        ...
        x_labels, y_values = calculate_geomean_speedups(dist_df, mode)
        ...
        output_file = f'geomean_speedup_by_{mode}_distribution_{distribution}.png'
        plot_bar_chart(x_labels, y_values, mode, output_file, custom_labels, figsize, sf_number, title_fontsize, label_fontsize, tick_fontsize, distribution)
        print(f"Bar chart saved as {output_file} for distribution {distribution}")
