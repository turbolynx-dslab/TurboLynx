...
def compute_series_for_dir(d: str, versions: List[int], query_groups: List[Tuple[str, List[str]]]) -> Dict[str, List[float]]:
    ...
    return series
def main():
    parser = argparse.ArgumentParser(description="Compare three directories of CSV results and plot 4 line graphs (PNG + PDF).")
    ...
    parser.add_argument("--versions", type=str, default="1-100", help="Version range, e.g. 1-100 or comma-separated list like 1,2,5")
    ...
    if n == 1:
        axes = [axes]
    # fig.suptitle("Geometric Mean of QueryExecutionTime by Version", fontsize=16)
    ...
    # plt.savefig(pdf_out, dpi=300, pad_inches=0)
    plt.savefig(pdf_out, dpi=800, bbox_inches='tight')
