...
for query_num in $queries; do
    ...
    warmup_output=$(timeout 20000 ../../build-release/tbgpp-client/TurboGraph-S62 --workspace:${database_path} --query:"$query_str" --index-join-only --join-order-optimizer:exhaustive)
    output_str=$(timeout 20000 ../../build-release/tbgpp-client/TurboGraph-S62 --workspace:${database_path} --query:"$query_str" --index-join-only --num-iterations:5 --join-order-optimizer:exhaustive)
    exit_status=$?
